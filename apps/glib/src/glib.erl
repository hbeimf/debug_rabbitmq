...
safe_reply(undefined, _Value) ->
  ok;
safe_reply(#{from := From, pid := _Pid}, Value) ->
  gen_server:reply(From, Value);
safe_reply(From, Value) ->
  gen_server:reply(From, Value).

term_to_int(Term) -> 
	term_to_int(Term, 100).
term_to_int(Term, Range) -> 
...
% 	{R4, S1} = rand:uniform_s(100, S0),
% 	{R4, S1}.

% r() -> 
% 	S0 = rand:seed_s(exsp),
% 	rand:uniform_s(100, S0).
...
rand1(Num) -> 
	rand(Num, exsp, rand_seed_exsp).	

rand2(Num) -> 
	rand(Num, exs1024s, rand_seed_exs1024s).	

rand3(Num) -> 
	rand(Num, exrop, rand_seed_exrop).	


rand(Num, T, Key) -> 
	SeedList = get(),
	S0 = get_by_key(Key, SeedList, rand:seed_s(T)),
	{R, S1} = rand:uniform_s(Num, S0),
	put(Key, S1),
	R.
...
%% 将Term 映射成 数字， 在 1.. Range 范转内
term_to_num(Term, Range) ->
	erlang:phash(Term, Range).
